...
class SLR_ME:
    def __init__(self, y, w, z, act_params, iteration, burnin, thinning, add_chains=False):
        ...
        gam0 ~ N(0, 10^2)
        gam1 ~ N(0, 10^2)
        sigma^2_x ~ IG(0.01, 0.01)
        sigma^2_v ~ IG(0.01, 0.01)
        sigma^2_d ~ IG(0.01, 0.01)
        sigma^2_ep ~ IG(0.01, 0.01)
        ...
        z: Observed covariate. n*1 dimension
    ...
    def _init_prior(self):
        ...
        self.s2_mu_x = 10**2
        self.s2_gam = 10**2
        self.A_x = self.B_x = self.A_v = self.B_v = self.A_d = self.B_d = self.A_ep = self.B_ep = 2
    ...
    def _sampling_s2_d(self):
        ...
        # parameters of full conditionals
        shape = A_d + n/2
        scale = B_d + np.dot(np.dot(X, gam).reshape(self.n) - z,
    ...
    def _sampling_gam(self):
        ...
        # parameters of full conditionals
        temp = np.linalg.inv(XTX / s2_d + np.identity(2) / s2_gam)
        mu = np.dot(temp,
                    np.dot(X.T, z)) / s2_d
        cov = temp
        # sampling from full conditionals
        ...
        # update posterior samples
        self.params['gam0'].append(gam0[0])
        self.params['gam1'].append(gam1[0])
    ...
    def _sampling_x(self):
        ...
        denom = beta1**2/s2_ep + 1/s2_v + gam1**2/s2_d + 1/s2_x
